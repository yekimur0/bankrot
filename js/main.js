...
/* harmony import */ var transfer_elements__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! transfer-elements */ "./node_modules/transfer-elements/dist/transfer-elements.esm.min.js");
...
(0,_functions_accordion__WEBPACK_IMPORTED_MODULE_3__.accordion)();

// Подключение свайпера

swiper__WEBPACK_IMPORTED_MODULE_6__["default"].use([swiper__WEBPACK_IMPORTED_MODULE_6__.Navigation, swiper__WEBPACK_IMPORTED_MODULE_6__.Pagination, swiper__WEBPACK_IMPORTED_MODULE_6__.Scrollbar]);
const REVIEWS_SWIPER = new swiper__WEBPACK_IMPORTED_MODULE_6__["default"]('.reviews__swiper', {
  navigation: {
    prevEl: '.reviews__btn-prev-1',
    nextEl: '.reviews__btn-next-1'
  },
  breakpoints: {
    320: {
      slidesPerView: 1,
...
const REVIEWS_SWIPER_VIDEO = new swiper__WEBPACK_IMPORTED_MODULE_6__["default"]('.reviews__video', {
  slidesPerView: 'auto',
  loop: true,
  centeredSlides: true,
  speed: 500
});
const BLOG_SWIPER = new swiper__WEBPACK_IMPORTED_MODULE_6__["default"]('.blog__swiper', {
  navigation: {
    prevEl: '.blog__btn-prev-1',
    nextEl: '.blog__btn-next-1'
  },
  breakpoints: {
    320: {
      slidesPerView: 1,
...
const STEPS_SWIPER = new swiper__WEBPACK_IMPORTED_MODULE_6__["default"]('.steps__swiper', {
  slidesPerView: 'auto',
  spaceBetween: 20
});
$(document).ready(function () {
  let currentStep = 0;
  const totalSteps = $('.calc-quiz__step').length;

  // Инициализация слайдера для выбора суммы долга
  $("#slider").slider({
    range: "min",
    min: 300000,
    max: 3000000,
    ...
    slide: function (event, ui) {
      $("#money-input").val(ui.value.toLocaleString());
    }
  });
  $("#money-input").val($("#slider").slider("value").toLocaleString());

  // Инициализация слайдера для выбора количества кредиторов
  $("#slider-2").slider({
    range: "min",
    min: 1,
    max: 100,
    ...
    step: 1,
    slide: function (event, ui) {
      $("#money-input-2").val(ui.value);
    }
  });
  $("#money-input-2").val($("#slider-2").slider("value"));

  // Показ текущего шага и скрытие остальных
  function showStep(step) {
    $(".calc-quiz__step").hide().eq(step).show();
    updateProgress(step);

    // Условие для отображения кнопки "Назад" на втором шаге
    if (step >= 1) {
  ...
  function updateProgress(step) {
    const progress = Math.floor((step + 1) / totalSteps * 100);
    $(".progress-text span").text(progress + "%");
    $(".progress-line-full").css("width", progress + "%");
  }

  // Показ начального шага
...
$(document).ready(function () {
  let currentStep = 0;
  const totalSteps = $('.instruct-quiz__step').length;
  ...
  $("#slider-3").slider({
    ...
    step: 10000,
    slide: function (event, ui) {
      $("#money-input-3").val(ui.value.toLocaleString());
    }
  });
  $("#money-input").val($("#slider-3").slider("value").toLocaleString());

  // Показ текущего шага и скрытие остальных
  function showStep(step) {
    $(".instruct-quiz__step").hide().eq(step).show();
    updateProgress(step);

    // Условие для отображения кнопки "Назад" на втором шаге
    if (step >= 1) {
      $("#btn-back-instruct").css("display", "flex");
    } else {
      $("#btn-back-instruct").css("display", "none");
    }
    if (step === 1) {
      $("#btn-next-instruct").css('display', 'none');
    } else if (step <= 1) {
      $("#btn-next-instruct").css('display', 'flex');
    }
    if (step === 1) {
      $('.instruct-quiz').css('max-width', '100%');
      $('.instruct-quiz').css('width', '100%');
      $('.instruct__wrapper').addClass('hidden-bg');
    } else if (step <= 0) {
      $('.instruct-quiz').css('max-width', '710px');
      $('.instruct-quiz').css('width', '100%');
      $('.instruct__wrapper').removeClass('hidden-bg');
    }
  }
  function updateProgress(step) {
    const progress = Math.floor((step + 1) / totalSteps * 100);
  ...
  // Переход к следующему шагу
  $("#btn-next-instruct").click(function () {
    if (currentStep < totalSteps - 1) {
      currentStep++;
      showStep(currentStep);
    }
  ...
  // Переход к предыдущему шагу
  $("#btn-back-instruct").click(function () {
    if (currentStep > 0) {
      currentStep--;
      showStep(currentStep);
    }
...
const modalArticleTrigger = document.querySelectorAll('.modal-article');
modalArticleTrigger.forEach(item => {
...
new transfer_elements__WEBPACK_IMPORTED_MODULE_7__["default"]({
  sourceElement: document.querySelector('.header__socials'),
  breakpoints: {
    768: {
...
new transfer_elements__WEBPACK_IMPORTED_MODULE_7__["default"]({
  sourceElement: document.querySelector('.header__actions'),
  breakpoints: {
    768: {
      targetElement: document.querySelector('.header__nav'),
      targetPosition: 1
...
const accordion = () => {
  ...
  function openAccordion(target) {
    const activeAccordion = document.querySelector('.faq__item--active');
    if (activeAccordion) activeAccordion.classList.remove('faq__item--active');
...
const listener = () => {
  document.body.addEventListener('click', e => {
    ...
    if (target.classList.contains('modal__close')) {
      ...
      if (parent.classList.contains('modal--active')) parent.classList.remove('modal--active');
    }
    if (target.classList.contains('modal-12')) {
      const modal = document.getElementById('modal-12');
      modal.classList.add('modal--active');
    }
    if (target.classList.contains('modal-11')) {
      const modal = document.getElementById('modal-11');
      modal.classList.add('modal--active');
    }
    if (target.classList.contains('modal-10')) {
      const modal = document.getElementById('modal-10');
      modal.classList.add('modal--active');
    }
    if (target.classList.contains('spec-mobile__dot')) changeCartInfo(target);
    function changeCartInfo(target) {
      const name = document.querySelector('.spec-mobile__cart-name');
      const status = document.querySelector('.spec-mobile__cart-status');
      const activeSpec = document.querySelector('.active-spec');
      const currentAttrName = target.dataset.name;
      const currentAttrStatus = target.dataset.status;
      const cartBlock = document.querySelector('.spec-mobile__cart');
      name.textContent = currentAttrName;
      status.textContent = currentAttrStatus;
      if (activeSpec) activeSpec.classList.remove('active-spec');
      target.classList.add('active-spec');
      if (!cartBlock.classList.contains('spec-mobile__cart--active')) cartBlock.classList.add('spec-mobile__cart--active');
    }
  });
};
listener();
